...
<div class="fixed-navbar">
    <div class="float-left">
        ...
        <h1 class="page-title">Update Expense</h1>
    </div>
</div>
		...
		<div class="row small-spacing">
			<div class="col-12">
				...
				<h4 class="box-title">Add Expense</h4>
					<form data-toggle="validator" action="{{ route('Expenses.update', $Expense->id) }}" method="POST" enctype="multipart/form-data">
					@csrf()
					@method('PATCH')
					<div class="container">
						<div class="form-group">
								<div class="row">
									...
									<div class="form-group col-md-4">
										<input type="date" class="form-control" id="dated" name="dated" value="{{ $Expense->dated }}" style="height: 30px!important;">
									</div>
								</div>
								...
								<div class="row mt-5">
									<table class="table" id="expensetable">
										<tbody>
											@foreach ($ExpensetoCategories as $expense )
											<tr>
												<td style="width:5%;">
												...
												<td style="width:25%" class="productTD">
													<select class="form-control select2_1 category" id="category_id" name="category_id[]" required>
														<option value="">Select</option>
														@foreach($categories as $category)
														<option value="{{$category->id}}" <?php echo ($category->id == $expense->category_id) ? 'selected' : ''; ?>>{{ $category->category }}</option>
														@endforeach
													</select>
												</td>
												<td style="width:40%">
													<input type="text" class="form-control qty" name="qty[]" placeholder="Detailed" value="{{ $expense->qty }}" style="height: 30px!important;">
													<div class="help-block with-errors"></div>
												</td>
												<td style="width:20%">
													<input type="number" step="0.01" class="form-control price" name="price[]" placeholder="Price" value="{{ $expense->price }}" style="height: 30px!important;">
												</td>
												<td style="width:20%">
							...
							<div class="row mt-5">
								<div class="form-group col-md-4">
									...
									<input type="number" step="0.01" class="form-control" id="amount"  value="{{ $Expense->amount }}" name="amount" required style="height: 40px;">
								</div>
								<div class="form-group col-md-4">
									<label>Cash Received</label>
									<input type="number" step="0.01" class="form-control" id="cashreceived" placeholder="Cash Received" value="{{ $Expense->cashreceived}}" name="cashreceived" required style="height: 40px;">
								</div>
								<div class="form-group col-md-4">
									<label>Previous Balance</label>
									<input type="number" step="0.01" class="form-control" id="pbalance" placeholder="P-Balance" value="{{ $Expense->pbalance}}" name="pbalance" required style="height: 40px;">
								</div>
								</div>
								<div class="row mt-5">
								<div class="form-group col-md-6">
									<label>Current Cash in Hand</label>
									<input type="number" step="0.01" class="form-control" id="cashinhand" placeholder="Current Cash in Hand" value="{{ $Expense->cashinhand }}" name="cashinhand" required style="height: 40px;">
								</div>
								<div class="form-group col-md-6">
									<label>Remaining Balance</label>
									<input type="number" step="0.01" class="form-control" id="remainingbalance" placeholder="Remaining Balance" value="{{ $Expense->remainingbalance }}" name="remainingbalance" required style="height: 40px;">
								</div>
								</div>
							...
							<div class="row mt-5">
								<div class="col-sm-3">
									<div class="input-group">
										<button type="submit" class="btn btn-primary btn-sm waves-effect waves-light">Update Expense</button>
									</div>
								</div>
							</div>
						</div>
