    						...
    						<div class="col-xl-12">
    							<form action="{{ route('filter-invoice') }}" method="GET" class="form-horizontal">
    								<div class="form-group row">
										...
										<div class="col-sm-2">
										<a href="{{ route('allinvoicesshow', ['data_key' => 'data_value']) }}" id="viewButton" class="btn btn-primary btn-sm">View</a>
										</div>
    								</div>
    							</form>
    						</div>
								...
								@if($Invoices)
									@foreach($Invoices as $Invoice)
										<tr>
											...
											if(isset($Invoice->gst_fixed)):
										    	$gst_fixed = $Invoice->gst_fixed;
										    else:
										    	$gst_fixed = 0;
										    endif;

											$Net_totalPrice = $totalPrice+$tax+$gst_fixed;
											...
											<td>
												<form action="{{ route('invoice.status', $Invoice->id) }}" method="post" target="_blank">
												@csrf
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        viewButton.addEventListener('click', function(e) {
            ...
            var baseUrl = "{{ route('allinvoicesshow') }}";
