									...
									@if($Branches)
										@foreach($Branches as $Branch)
											<option value="{{ $Branch->branch_name }}"
												@if(old('branch') == $Branch->branch_name || (isset($user->branch) && $user->branch == $Branch->branch_name))
													selected
												@endif
											>
												{{ $Branch->branch_name }}
...
$( document ).ready(function() {
	...
	$("#customerTable").on('click', '.addmore', function(){
		...
		data +='<div class="form-group col-md-2" style="text-align: left; margin-left: 10px;margin-top: 10px;">';
